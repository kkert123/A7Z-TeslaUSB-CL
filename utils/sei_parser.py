...
def _parse_sei_nal(nal):
    if len(nal) < 10:
        return None
    # 官方（teslamotors/dashcam）只处理 user_data_unregistered 类型的 SEI：
    # AVC NAL header 第二字节为 payloadType=5
    if nal[1] != 0x05:
        return None
    i = 2
    ...
    try:
        ...
        return {'speed_kmh':round(m.vehicle_speed_mps*3.6,1),
            ...
            'latitude_deg':round(m.latitude_deg,6),'longitude_deg':round(m.longitude_deg,6),
            'heading_deg':round(m.heading_deg,1),
            'linear_acceleration_mps2_x':round(m.linear_acceleration_mps2_x,3),
            'linear_acceleration_mps2_y':round(m.linear_acceleration_mps2_y,3),
            'linear_acceleration_mps2_z':round(m.linear_acceleration_mps2_z,3)}
    except:
        pass
...
def _manual_sei_parse(payload):
    res = {'speed_kmh':0,'gear_state':'?','steering_wheel_angle':0,
        ...
        'linear_acceleration_mps2_x':0.0,'linear_acceleration_mps2_y':0.0,
    ...
    while p < len(payload):
        ...
        elif wt == 1:  # fixed64 (double) — GPS / heading / 加速度
            if p + 8 <= len(payload):
                v = struct.unpack('<d', payload[p:p+8])[0]; p += 8
                if fn == 11: res['latitude_deg'] = round(v,6)
                elif fn == 12: res['longitude_deg'] = round(v,6)
                elif fn == 13: res['heading_deg'] = round(v,1)
                elif fn == 14: res['linear_acceleration_mps2_x'] = round(v,3)
                elif fn == 15: res['linear_acceleration_mps2_y'] = round(v,3)
                elif fn == 16: res['linear_acceleration_mps2_z'] = round(v,3)
