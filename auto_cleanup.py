...
class AutoCleaner:
    ...
    def cleanup_old_versions(self, keep: int = 2) -> int:
        ...
        # 收集所有版本目录 + 版本元组
        versions = []  # [(ver_tuple, dir_path)]
        try:
            for name in os.listdir(deploy_base):
                m = pattern.match(name)
                if not m:
                    continue
                ver = tuple(int(x) for x in m.groups() if x is not None)
                d = os.path.join(deploy_base, name)
            ...
            logger.warning("版本目录清理: 扫描失败 %s", e)
            return 0

        if len(versions) <= keep - 1:
            return 0  # 含当前在内不足 keep 个，无需清理

        # 按版本号降序（新 → 旧），保留最大的 keep-1 个（当前已单独保护）
        versions.sort(key=lambda x: x[0], reverse=True)
        to_delete = versions[keep - 1:]
    ...
    def run(self) -> Dict:
        ...
        # 1. 始终清理过期预览/临时/日志 + 旧版本目录（v0.3.1.34：保留 5 个版本）
        ...
        if deleted_versions > 0:
            self.stats["actions"].append(f"清理旧版本目录: {deleted_versions} 个")
            logger.info(f"清理旧版本目录: {deleted_versions} 个")
