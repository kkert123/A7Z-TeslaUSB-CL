...
class SentryNotifyQueue:
    ...
    def process_pending(self) -> int:
        ...
        if sent_count > 0:
            ...
            self._notify_backfill_summary(sent_count, len(remaining))
        return sent_count

    def _notify_backfill_summary(self, sent_count: int, remaining: int):
        """补发完成后发送汇总通知，让用户感知"断网补发流程已触发"。

        背景：断网恢复通知(system_monitor)与补发队列解耦，用户只看得到
        "✅ 网络已恢复"而不知道离线期间的事件通知已被补发。此处补一条文本汇总。
        """
        try:
            notifier = self._get_notifier()
            if not notifier:
            ...
            text = (
                "🔔 断网补发\n"
                f"离线期间共有 {sent_count} 条哨兵事件通知已补发"
                + (f"\n（另有 {remaining} 条仍待重试）" if remaining else "")
            )
            notifier.send_text(text)
            logger.info(f"已发送补发汇总通知: {sent_count} 条")
        except Exception as e:
            logger.error(f"发送补发汇总通知失败: {e}")
