...
remount_all_ro() {
    ...
    for i in "${!LUN_DEVICES[@]}"; do
        ...
        [ -z "$mp" ] && continue
        mkdir -p "$mp"
        # 检查是否已 RO 挂载，是则跳过（保持已有挂载）
        if mountpoint -q "$mp" 2>/dev/null; then
            fm=$(findmnt -n -o OPTIONS "$mp" 2>/dev/null)
            if echo "$fm" | grep -q "ro"; then
                green "    $mp (ro) 已挂载，保持"
                continue
            fi
        fi
        # 先尝试 umount（可能之前已挂载 RW）
...
start_gadget() {
    ...
    for dev in "${LUN_DEVICES[@]}"; do
        ...
        # TeslaCam (nvme0n1p2) 保持 RO 挂载，不卸载（允许与 Gadget LUN 共存）
        if [ "$dev" = "/dev/nvme0n1p2" ]; then
            fm=$(findmnt -n -o OPTIONS "$dev" 2>/dev/null)
            if echo "$fm" | grep -q "ro"; then
                green "    保留 TeslaCam RO 挂载（与 Gadget 共享）"
                continue
            fi
        fi
    ...
    for dev in "${LUN_DEVICES[@]}"; do
        sn=$(basename "$dev")
        # TeslaCam 允许 RO 挂载与 Gadget 共存
        if [ "$dev" = "/dev/nvme0n1p2" ]; then
            fm=$(findmnt -n -o OPTIONS "$dev" 2>/dev/null)
            if echo "$fm" | grep -q "ro"; then
                green "    TeslaCam RO 挂载正常，继续"
                continue
            fi
        fi
