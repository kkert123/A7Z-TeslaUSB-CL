...
# ── CPU 阈值 ──
CPU_START_THRESHOLD = 50.0   # CPU 低于此值开始生成（A7Z 基线 40-60%）
...
MAX_RETRIES = 3
PENDING_THRESHOLD = 3         # 队列积压阈值：累计 3 个就触发生成（降低门槛）
MAX_BATCH_PER_CYCLE = 3        # 每扫描周期最多处理 N 个
            ...
            if folder_name == 'RecentClips':
                ...
                for session_id, info in sessions.items():
                    if info['newest_mtime'] > active_cutoff:
                        continue  # 跳过最近 2 分钟
                    # 至少 2 个摄像头文件就绪即可生成缩略图（前/后/左/右至少一对）
                    if len(info['files']) < 2:
