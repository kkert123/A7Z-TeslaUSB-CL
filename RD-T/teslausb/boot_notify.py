...
def get_boot_time_str() -> str:
    ...
    try:
        ...
        # teslausb-mode 服务激活时间（USB Gadget 就绪 = 启动完成）
        result = subprocess.run(
            ["systemctl", "show", "teslausb-mode.service",
             "--property=ActiveEnterTimestampMonotonic"],
            capture_output=True, text=True, timeout=5
        )
        if result.returncode == 0:
            # ActiveEnterTimestampMonotonic 是 monotonic 时间（秒）
            val = result.stdout.strip().split("=")[1]
            ready_us = int(val)  # 微秒
            ready_s = ready_us / 1_000_000  # 转为秒
        else:
            # 回退：用当前 uptime（不够精确）
...
def get_wifi_info() -> Dict:
    """获取 WiFi 信息（优先 nmcli，回退 iwconfig）"""
    info = {"connected": False, "ssid": "", "signal_pct": 0}
    
    # 方法1: nmcli (NetworkManager)
    try:
        result = subprocess.run(
        ...
        if info["connected"]:
            ...
            # 输出格式: "yes:100" (active:signal, SSID不在这个输出中)
    ...
    if not info["connected"]:
        try:
            ...
            if ssid_match and ssid_match.group(1) and ssid_match.group(1) != "off/any":
                info["connected"] = True
...
def get_service_status() -> Dict[str, bool]:
    """获取服务状态"""
    services = {}
    for svc in ["teslausb-web", "teslausb-mode", "teslausb-fsck.timer", "teslausb-io-tune"]:
        try:
            r = subprocess.run(
