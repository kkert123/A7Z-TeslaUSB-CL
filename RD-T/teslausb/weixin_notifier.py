...
class WeixinNotifier:
    ...
    def send_boot_notification(self, boot_time: str, cpu_info: dict,
                              ...
                              wifi_info: dict = None, local_ip: str = None,
                              tailscale_ip: str = None,
                              services: dict = None, nvme_temp: float = 0.0) -> bool:
        """
        发送开机通知
        """
        lines = ["🚀 TeslaUSB A7Z 已启动", ""]
        
        # 启动时间
        lines.append(f"⏰ {boot_time}")
        
        # CPU + NVMe 温度
        cpu_parts = []
        if cpu_info.get('model'):
            cpu_parts.append(cpu_info['model'])
        if cpu_info.get('freq_mhz', 0) > 0:
            cpu_parts.append(f"{cpu_info['freq_mhz']}MHz")
        if cpu_info.get('temp_c', 0) > 0:
            cpu_parts.append(f"{cpu_info['temp_c']}°C")
        if cpu_parts:
            lines.append(f"🖥️ {' · '.join(cpu_parts)}")
        
        if nvme_temp and nvme_temp > 0:
            lines.append(f"🌡️ NVMe {nvme_temp}°C")
        
        # 内存 + SWAP
        if memory_info:
            lines.append(f"💾 内存 {memory_info.get('used_mb',0)}/{memory_info.get('total_mb',0)}MB ({memory_info.get('pct',0)}%)")
            if memory_info.get('swap_total_mb', 0) > 0:
                lines.append(f"📀 SWAP {memory_info['swap_used_mb']}/{memory_info['swap_total_mb']}MB ({memory_info.get('swap_pct',0)}%)")
        ...
        if disk_info:
            ...
            for d in disk_info:
                if d.get('mounted'):
                    ...
                    lines.append(f"  {d['name']} {d['used_gb']}/{d['total_gb']}GB ({pct}%) {icon}")
                else:
                    lines.append(f"  {d['name']} 未挂载")
        
        # 网络
        if wifi_info and wifi_info.get('connected'):
            lines.append(f"📶 WiFi {wifi_info['ssid']} ({wifi_info.get('signal_pct',0)}%)")
        else:
            lines.append(f"📶 WiFi 未连接")
        if local_ip and local_ip != "N/A":
            # 去掉接口名前缀（如 "wlan0 192.168.0.102" → "192.168.0.102"）
            ip_only = local_ip.split()[-1] if ' ' in local_ip else local_ip
            lines.append(f"🏠 本地IP {ip_only}")
