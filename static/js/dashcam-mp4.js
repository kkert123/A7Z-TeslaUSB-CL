...
class DashcamMP4 {
    ...
    /** Parse video frames with SEI metadata. Frames have `time` (seconds) for HUD sync. */
    parseFrames(SeiMetadata) {
        ...
        let frameCumMs = 0;  // cumulative time in ms
        ...
        while (cursor + 4 <= end) {
            ...
            if (type === 7) currentSps = data; // SPS
                ...
                const dur = config.durations[frames.length] || 33.3667; // ~30fps default
                const time = frameCumMs / 1000;
                frames.push({
                    index: frames.length,
                    keyframe: type === 5,
                    data,
                    ...
                    sps: currentSps,
                    pps: currentPps,
                    time  // seconds — used for HUD sync
                });
                pendingSei = null;
                frameCumMs += dur;
            }
