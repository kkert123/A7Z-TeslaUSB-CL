...
def api_player_video():
    ...
    # 只允许访问 TeslaCam 目录内的文件，禁止绝对路径（防任意文件读取）
    teslacam_root = os.path.realpath('/mnt/teslacam/TeslaCam')
    full_path = path if path.startswith('/') else os.path.join(teslacam_root, path)
    full_path = os.path.realpath(full_path)
    if not full_path.startswith(teslacam_root + os.sep) and full_path != teslacam_root:
        return 'Invalid path', 400
