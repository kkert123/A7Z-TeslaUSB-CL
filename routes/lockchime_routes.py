...
def lockchime_upload():
    ...
    try:
        ...
        # 防路径穿越：只用 basename（客户端可控 filename 可能含 ../../）
        safe_name = os.path.basename(file.filename)
        if not safe_name.lower().endswith('.wav'):
            return jsonify({'success': False, 'error': f'仅支持 WAV 格式'}), 400
        save_path = os.path.join(LOCKCHIME_DIR, safe_name)
        file.save(save_path)
        current_app.logger.info(f"🔔 [LockChime] 上传: {safe_name}")
        return jsonify({'success': True, 'filename': safe_name,
                        'message': f'{safe_name} 已上传'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
